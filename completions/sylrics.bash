_sylrics() {
  ...
  case "$prev" in
    ...
    gaps) COMPREPLY=( $(compgen -W 'off dots-beta' -- "$cur") ) ;;
    click-seek) COMPREPLY=( $(compgen -W 'on off' -- "$cur") ) ;;
    ...
    config) COMPREPLY=( $(compgen -W 'path list get set edit restore reset' -- "$cur") ) ;;
    cache) COMPREPLY=( $(compgen -W 'info clear' -- "$cur") ) ;;
    bridge) COMPREPLY=( $(compgen -W 'install status' -- "$cur") ) ;;
    *) COMPREPLY=( $(compgen -W 'play demo gaps click-seek config theme preset reading typing highlight font source visualizer control doctor cache bridge --source --config --version --help' -- "$cur") ) ;;
  esac
}
